...
# lies behind the letters there: a blue panel, where everything that is not blue is lettering, or a
# flat area (a ribbon, a panel), where the whole box is refilled from its edge so that the letters'
# soft shadows go too; a flat box must lie inside the area, clear of its border lines.
BOXES = {
    'tooltip_play': [((0.2, 0.12, 0.8, 0.56), 'blue')],
    'settings_popup': [((0.25, 0.03, 0.75, 0.176), 'flat')],
    'level_complete': [((0.16, 0.026, 0.84, 0.108), 'flat'), ((0.085, 0.42, 0.345, 0.53), 'flat'), ((0.085, 0.545, 0.352, 0.67), 'flat')],
}
...
def background(rgb, kind):
    r, g, b = rgb[..., 0], rgb[..., 1], rgb[..., 2]
    if kind == 'blue':
        return (b > 195) & (r < 195)
    if kind == 'flat':
        return np.zeros(rgb.shape[:2], bool)  # nothing in the box is kept
...
def erase(name):
    ...
    for (l, t, r, b), kind in BOXES[name]:
        ...
        box |= inside
        mask |= inside & ~background(a[..., :3], kind)
    grown = np.array(Image.fromarray((mask * 255).astype(np.uint8)).filter(ImageFilter.MaxFilter(9))) > 0
